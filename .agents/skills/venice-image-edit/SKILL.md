...
description: Change an image that already exists, with Venice. Use to edit one by prompt, composite several into one, upscale 2x or 4x, or cut the background out to a transparent PNG. Every response is binary, so results land on disk.
---
...
- Because the response is bytes, every call below pipes to a path with `-o`. Use
  `-sf` too: without `-f`, curl exits 0 on a 400 and writes the JSON error body
  into your `.png`. `file out.png` tells you which one you got.
- Prefer an **HTTPS URL or file upload** for the input over base64. Base64 of a
  real photo is millions of characters; pasting one into a request you compose by
  hand costs the same context a generated image would.
...
```bash
curl -sf https://api.venice.ai/api/v1/image/edit \
...
```bash
curl -sf https://api.venice.ai/api/v1/image/upscale \
  -H "Authorization: Bearer $VENICE_API_KEY" \
  -F image=@public/hero.webp \
  -F scale=4 \
  -F creativity=0.01 \
  -o public/hero@4x.png
```
...
Shown here as `multipart/form-data`, which is the way to send a local file without
base64-encoding it first — upscale takes base64 or a file upload, and unlike edit
and background-remove it has no URL field. The JSON form is the same three fields
with `"image"` as a base64 string. Response is binary `image/png` either way.
...
```bash
# With a URL — preferred
curl -sf https://api.venice.ai/api/v1/image/background-remove \
  -H "Authorization: Bearer $VENICE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"image_url": "https://example.com/photo.jpg"}' \
  -o public/cutout.png

# With base64, when the source is a local file
curl -sf https://api.venice.ai/api/v1/image/background-remove \
  -H "Authorization: Bearer $VENICE_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"image\": \"$(base64 -i public/photo.jpg)\"}" \
  -o public/cutout.png
```
